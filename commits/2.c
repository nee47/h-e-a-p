...
void heapify(heap_t* heap, size_t n){
  for(size_t i = n/2; i > 0; i--){
    down_heap(heap, n, i-1);
  }
}
...
bool redimensionar(heap_t* heap, size_t nuevo_largo ){
  printf("voy a  redimensioanr large %zu \n", nuevo_largo);
  void** aux = realloc(heap->datos, (sizeof(heap->datos) * nuevo_largo));
  //if (!aux) return false;
  //free(heap->datos);
  heap->datos = aux; 
  heap->largo = nuevo_largo;
  printf("termine de redimensionar\n");
  return true;
}
...
bool heap_encolar(heap_t *heap, void *elem){
  if(heap->tam > heap->largo + 2){
    //size_t nuevo_largo = heap->largo * REDIMEN ;
    printf("redimensioanr largo %zu\n", heap->largo * REDIMEN);    
